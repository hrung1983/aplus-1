...
if($id!=""){
  $sqlcmd = "select * from tbl_user where id = $id";
  $db->sql($sqlcmd);
$res = $db->getResult();		
	foreach($res as $data){
		$fname = @$data["fname"];
		...
		$status_position = $data["status_position"];
	}	
}
        ...
        <div class="form-group">
          ...
          <div class="col-sm-10">
            <select id="status_position" name="status_position" class="form-control">
              ...
              <option value="User" <?php if(@$status_position=="User") echo "selected";?>>ผู้ใช้งาน</option>
            </select>
        </div>
        </div>
    </div>
